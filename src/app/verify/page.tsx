...
const VerifyPage = () => {
	...
	const handleSubmit = async (e: React.FormEvent<HTMLElement>) => {
		...
		setLoading(true);
		try {
			const { data } = await axios.post(`http://localhost:5000/api/v1/user/verify`, { email, otp: otpString });
			alert(data.message);
			Cookies.set('token', data.tpken, {
				expires: 15,
				secure: false,
				path: '/',
			});
			...
			<div className='max-w-md w-full'>
				<div className='bg-gray-800 border border-gray-700 rounded-lg p-8'>
					...
					<form className='space-y-6' onSubmit={handleSubmit}>
						<div>
							<label className='block text-sm font-medium text-gray-300 mb-4 text-center'>Enter 6 Digit OTP here</label>
